...
class UvPreprocessor:
    def __init__(self, conf): # Store configuration
        self.conf = conf
    ...
    def preprocess_uv(self):
        ...
        # Mask eclipse and transit samples
        mask = (df["eclipse"] | df["lunar_transit"] | df["geocorona"])
        df.loc[mask, "value"] = np.nan

        # Pivot lines into columns
        df = df.pivot_table(index="time_tag", columns="line", values="value")
        df.columns = [f"euv_{col}" for col in df.columns] # Prefix EUV columns
        
        # Expand to one-minute frequency
        df = df.resample('1min').asfreq()

        # Mark rows with missing EUV data
        df['euv_is_missing'] = df.isna().all(axis=1).astype(int)

        # Select EUV value columns
